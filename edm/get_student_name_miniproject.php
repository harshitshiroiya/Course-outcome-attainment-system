...
$count_qry = "SELECT * FROM `marks_miniproject` WHERE `batch`='$batch' AND `sem_id`='$intake' AND `course_id`='$courseid'";
...
echo "<form action='marks_miniproject_save.php' class='form-horizontal' enctype='multipart/form-data' method='POST'>";
...
echo "<thead>";    
    ...
    for($y=0;$y<sizeof($miniproject);$y++)
          {
            echo "<th>".$miniproject[$y]."</th>";
          }
...
//    echo $intake_query;
    ...
    for($y=0;$y<sizeof($miniproject);$y++)
              {
                echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$miniproject[$y]."[]' class='form-control1' value=0 max='20' min='0' readonly oninput='validity.valid || (value=0);'></div></div></div></td>";
              }
...
else
{
  ...
  if (mysqli_num_rows($student_result) > 0) 
    ...
    for($y=0;$y<sizeof($miniproject);$y++)
              {
                echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$miniproject[$y]."[]' class='form-control1' value=0 max='20' min='0'  oninput='validity.valid || (value=0);'></div></div></div></td>";
              }
         // echo "<thead><th>".$student_row['std_roll_no']."</th></thead>";
         // echo "<thead><th>".$student_row['student_name']."</th></thead>";
       }
