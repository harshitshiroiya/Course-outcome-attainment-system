	...
	<div class="main-content">
		...
		<div class="row">
			<h3 class="title1"><b>CO - Internal Assesment Mapping</b> (<?php echo $_SESSION['course_id']; ?>):</h3>
				...
				<form id="co_am_form" onsubmit="return checkTotal()" action="ia_co_save.php" class="form-horizontal" enctype="multipart/form-data" method="POST">
				...
				{
					$ia_result = mysqli_query($conn,$ia_qry);

					if(mysqli_num_rows($ia_result)>0)
					{
						$row = mysqli_fetch_assoc($ia_result);

						for ($x = 1; $x <= 2; $x++)
						{
							for( $i ='a'; $i <= 'f'; $i++ ) 
							{​​​​​
								if(!is_null($row['IA'.$x.'_'.$x.$i]))
								{
									$am[]='IA'.$x.'_'.$x.$i;
									$am_nm[]=$row['IA'.$x.'_'.$x.$i];
				...
				<?php
				$no=1;
				if (mysqli_num_rows($co_ia_result) > 0) 
				{
					while($co_am_row = mysqli_fetch_assoc($co_ia_result))
					{
						$sum=0;
						echo "<tr>";
						echo "<td>CO".$no."</td>";
