...
<body id="mainclass" class="cbp-spmenu-push-toright">
	<div class="main-content">
		...
		<div id="page-wrapper">
		<div class="row">
			<h3 class="title1"><b>LO - Assignment Mapping</b> (<?php echo $_SESSION['course_id']; ?>):</h3>
				<div class="form-three widget-shadow">
				<div class=" panel-body-inputin">
				<form id="lo_am_form" onsubmit="return checkTotal()" action="assignment_lo_save.php" class="form-horizontal" enctype="multipart/form-data" method="POST">
				...
				$assignment_qry="SELECT `A1_1`,`A1_2`,`A1_3`,`A1_4`,`A1_5`,`A2_1`,`A2_2`,`A2_3`,`A2_4`,`A2_5` FROM `assignment_lo` WHERE `course_id`='$course_id'";
				$lo_assignment_qry="SELECT * FROM `assignment_lo` WHERE `course_id`='$course_id'";
				
				// echo ("SELECT * FROM `assignmnet_lo` WHERE `course_id`='$course_id'");

				$lo_assignment_result = mysqli_query($conn,$lo_assignment_qry);
				...
				<?php
				$no=1;
				if (mysqli_num_rows($lo_assignment_result) > 0) 
				{
					while($lo_assignment_row = mysqli_fetch_assoc($lo_assignment_result))
					{
						$sum=0;
						echo "<tr class='table_row'>";
						echo "<td>CO".$no."</td>";
						echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='text' name='lo_id[]' class='form-control1' value=".$lo_assignment_row['lo_id']." readonly></div></div></div></td>";
						//echo implode(',', $assignment);
						for($y=0;$y<sizeof($assignment);$y++)
						{
							echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$assignment[$y]."[]' value='".$lo_assignment_row[$assignment[$y]]."' class='form-control1' value=0 max='100' min='0' onchange='calculateSum(this)' oninput='validity.valid || (value=0);' readonly style='background-color:#dfdfdf;'></div></div></div></td>";
							$sum+=$lo_assignment_row[$assignment[$y]];
						}
