...
<body id="mainclass" class="cbp-spmenu-push-toright">
	<div class="main-content">
		...
		<div id="page-wrapper">
		<div class="row">
			<h3 class="title1"><b>LAB EXIT FORM</b> (<?php echo $_SESSION['course_id']; ?>):</h3>
				<div class="form-three widget-shadow">
				<div class=" panel-body-inputin">
				<form id="co_am_form" onsubmit="return checkTotal()" action="lo_exit_save.php" class="form-horizontal" enctype="multipart/form-data" method="POST">
				...
				$exit_qry="SELECT `question_no`,`questions`,`lo_no` FROM `lab_exit` WHERE `course_id`='$course_id'";
				$lo_exit_qry="SELECT * FROM `lab_exit` WHERE `course_id`='$course_id'";
				//echo $co_am_qry;

				$lo_exit_result = mysqli_query($conn,$lo_exit_qry);
				...
				<?php
				$no=1;
				if (mysqli_num_rows($lo_exit_result) > 0) 
				{
					while($lo_exit_row = mysqli_fetch_assoc($lo_exit_result))
					{
						// echo implode(',',$exit);
						// echo "<br>";
						for($y=0;$y<sizeof($exit);$y++)
						{
							...
							echo "<td><input type='text' name='".$exit[$y]."[]' value='".$lo_exit_row[$exit[$y]]."' class='form-control1' value=0 max='100' min='0' onchange='calculateSum(this)' oninput='validity.valid || (value=0);' readonly style='background-color:#dfdfdf;'></td>";
