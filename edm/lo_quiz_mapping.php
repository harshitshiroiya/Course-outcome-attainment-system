	...
	<div class="main-content">
		...
		<div class="row">
			<h3 class="title1"><b>LO - Quiz Mapping</b> (<?php echo $_SESSION['course_id']; ?>):</h3>
				...
				<form id="co_am_form" onsubmit="return checkTotal()" action="lo_quiz_save.php" class="form-horizontal" enctype="multipart/form-data" method="POST">
				...
				$quiz_qry="SELECT `quiz_1`,`quiz_2`,`quiz_3`,`quiz_4`,`quiz_5` FROM `quiz_lo` WHERE `course_id`='$course_id'";
				$lo_quiz_qry="SELECT * FROM `quiz_lo` WHERE `course_id`='$course_id'";
				//echo $co_am_qry;
				...
				<?php
				$no=1;
				if (mysqli_num_rows($lo_quiz_result) > 0) 
				{
					while($lo_quiz_row = mysqli_fetch_assoc($lo_quiz_result))
					{
						$sum=0;
						echo "<tr>";
						echo "<td>LO".$no."</td>";
						$no=$no+1;
						echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='text' name='lo_id[]' class='form-control1' value=".$lo_quiz_row['lo_id']." readonly></div></div></div></td>";
						for($y=0;$y<sizeof($quiz);$y++)
						{
							echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$quiz[$y]."[]' value='".$lo_quiz_row[$quiz[$y]]."' class='form-control1' value=0 max='100' min='0' onchange='calculateSum(this)' oninput='validity.valid || (value=0);' readonly style='background-color:#dfdfdf;'></div></div></div></td>";
							$sum+=$lo_quiz_row[$quiz[$y]];
						}
