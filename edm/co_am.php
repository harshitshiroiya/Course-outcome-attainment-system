...
<body id="mainclass" class="cbp-spmenu-push-toright">
	<div class="main-content">
		...
		<div id="page-wrapper">
		<div class="row">
			<h3 class="title1"><b>Contribution of each Assessment Method (AM) in every CO</b> (<?php echo $_SESSION['course_id']; ?>):</h3>
				<div class="form-three widget-shadow">
				<div class=" panel-body-inputin">
				<form id="co_am_form" onsubmit="return checkTotal()" action="co_am_save.php" class="form-horizontal" enctype="multipart/form-data" method="POST">
				...
				$am_qry="SELECT `am1`,`am2`,`am3`,`am4`,`am5`,`am6`,`am7`,`am8`,`am9` FROM `course_am` WHERE `course_id`='$course_id'";
				$co_am_qry="SELECT * FROM `co_am` WHERE `course_id`='$course_id'";
				//echo $co_am_qry;

				$co_am_result = mysqli_query($conn,$co_am_qry);
				...
				{
					$am_result = mysqli_query($conn,$am_qry);
					//echo $am_result;
					if(mysqli_num_rows($am_result)>0)
					{
						$row = mysqli_fetch_assoc($am_result);
						for ($x = 1; $x <= 9; $x++)
						{
							if(!is_null($row['am'.$x]))
							{
				...
				<?php
				$no=1;
				if (mysqli_num_rows($co_am_result) > 0) 
				{
					while($co_am_row = mysqli_fetch_assoc($co_am_result))
					{
						$sum=0;
						echo "<tr>";
						echo "<td>CO".$no."</td>";
						$no=$no +1;
						echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='text' name='co_id[]' class='form-control1' value=".$co_am_row['co_id']." readonly></div></div></div></td>";
						for($y=0;$y<sizeof($am);$y++)
						{
							echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$am[$y]."[]' value='".$co_am_row[$am[$y]]."' class='form-control1' value=0 max='100' min='0' onchange='calculateSum(this)' oninput='validity.valid || (value=0);' readonly style='background-color:#dfdfdf;'></div></div></div></td>";
							$sum+=$co_am_row[$am[$y]];
						}
						echo "<td><div class='form-group'><div class='col-md-6'><input type='number' name='total[]' class='form-control1' value='".$sum."' max='100' min='100' readonly style='background-color:#C0C0C0;'></div><div class='col-md-6'><button type='button' class=\"btn btn-success\" onclick=\"editRow(this)\"><i class=\"fa fa-pencil-square-o\"></i></button></div></div></td>";
				...
				else
				{
					if (mysqli_num_rows($co_result) > 0) 
					{
						while($co_row = mysqli_fetch_assoc($co_result))
						{
							...
							for($y=0;$y<sizeof($am);$y++)
							{
								echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$am[$y]."[]' class='form-control1' value=0 max='100' min='0' onchange='calculateSum(this)' oninput='validity.valid || (value=0);'></div></div></div></td>";
							}
