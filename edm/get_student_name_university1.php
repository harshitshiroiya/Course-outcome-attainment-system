...
$student_qry="SELECT `division_details`.`std_id`,`sem_id`,`std_roll_no`,`student_table`.`student_name`,`intake`.`sem_id` FROM `student_table` , `division_details`,`intake` WHERE `division_details`.`sem_id`=`intake`.`sem_id` AND `division_details`.`std_id`=`student_table`.`std_id`";
...
//    echo $intake_query;
    ...
    for($y=0;$y<sizeof($university);$y++)
              {
                echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$university[$y]."[]' class='form-control1' value=0 max='20' min='0' readonly oninput='validity.valid || (value=0);'></div></div></div></td>";
              }
...
else
{
  ...
  if (mysqli_num_rows($student_result) > 0) 
    ...
    for($y=0;$y<sizeof($university);$y++)
              {
                echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$university[$y]."[]' class='form-control1' value=0 max='20' min='0'  oninput='validity.valid || (value=0);'></div></div></div></td>";
              }
         // echo "<thead><th>".$student_row['std_roll_no']."</th></thead>";
         // echo "<thead><th>".$student_row['student_name']."</th></thead>";
       }
