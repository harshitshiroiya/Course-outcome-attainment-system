	...
	<div class="main-content">
		...
		<div class="row">
			<h3 class="title1"><b>LO - PO PSO Mapping</b> (<?php echo $_SESSION['course_id']; ?>):</h3>
				...
				<form id="co_am_form" onsubmit="return checkTotal()" action="po_pso_mapping_save_of_lo.php" class="form-horizontal" enctype="multipart/form-data" method="POST">
				...
				$po_mapping="SELECT `pso1`,`pso2`,	`po1`,`po2`,`po3`,`po4`,`po5`,`po6`,`po7`,`po8`,`po9`,`po10`,`po11`,`po12` FROM `po_pso_lo` WHERE `course_id`='$course_id'";
				$po_mapping_qry="SELECT * FROM `po_pso_lo` WHERE `course_id`='$course_id'";
				// echo $ia_qry;

				$po_mapping_result = mysqli_query($conn,$po_mapping_qry);
				...
				{
					$po_pso_mapping_result = mysqli_query($conn,$po_mapping);

					$fieldinfo = $po_pso_mapping_result -> fetch_fields();
					...
					// echo implode(',', $am_nm);
					if(mysqli_num_rows($po_pso_mapping_result)>0)
					{
						$row = mysqli_fetch_assoc($po_pso_mapping_result);
				...
				<?php
				$no=1;
				if (mysqli_num_rows($po_mapping_result) > 0) 
				{
					while($lo_am_row = mysqli_fetch_assoc($po_mapping_result) )
					{
						$sum=0;
