...
<body id="mainclass" class="cbp-spmenu-push-toright">
	<div class="main-content">
		...
		<div id="page-wrapper">
		<div class="row">
			<h3 class="title1"><b>LO - PO PSO Mapping</b> (<?php echo $_SESSION['course_id']; ?>):</h3>
				<div class="form-three widget-shadow">
				<div class=" panel-body-inputin">
				<form id="co_am_form" onsubmit="return checkTotal()" action="lo_po_mapping_save.php" class="form-horizontal" enctype="multipart/form-data" method="POST">
				...
				$lo_mapping="SELECT `pso1`,`pso2`,	`po1`,`po2`,`po3`,`po4`,`po5`,`po6`,`po7`,`po8`,`po9`,`po10`,`po11`,`po12` FROM `lo_po_pso` WHERE `course_id`='$course_id'";
				$lo_mapping_qry="SELECT * FROM `lo_po_pso` WHERE `course_id`='$course_id'";
				
				$lo_mapping_result = mysqli_query($conn,$lo_mapping_qry);
				...
				{
					$lo_po_mapping_result = mysqli_query($conn,$lo_mapping);

					$fieldinfo = $lo_po_mapping_result -> fetch_fields();
					...
					// echo implode(',', $am_nm);
					if(mysqli_num_rows($lo_po_mapping_result)>0)
					{
						$row = mysqli_fetch_assoc($lo_po_mapping_result);

						for ($x = 0; $x < sizeof($am_nm); $x++)
						{
							if(!is_null($row[$am_nm[$x]]))
							{
								$am[]=$row[$am_nm[$x]];
							}
				...
				<?php
				$no=1;
				if (mysqli_num_rows($lo_mapping_result) > 0) 
				{
					while($lo_am_row = mysqli_fetch_assoc($lo_mapping_result) )
					{
						$sum=0;
						echo "<tr class ='table_row'>";
                        echo "<td>CO".$no."</td>";
                        $no = $no +1;
						echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='text' name='co_id[]' class='form-control1' value=".$lo_am_row['lo_id']." readonly></div></div></div></td>";
						for($y=0;$y<sizeof($am_nm);$y++)
						{
							echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$am_nm[$y]."[]' value='".$lo_am_row[$am_nm[$y]]."' class='form-control1' value=0 max='100' min='0' onchange='calculateSum(this)' oninput='validity.valid || (value=0);' readonly style='background-color:#dfdfdf;'></div></div></div></td>";
							$sum+=$lo_am_row[$am_nm[$y]];
						}
				...
				else
				{
					if (mysqli_num_rows($lo_result) > 0) 
					{
						while($lo_row = mysqli_fetch_assoc($lo_result))
						{
							...
							for($y=0;$y<sizeof($am_nm);$y++)
							{
                                echo "<td><div class='form-group'><div class='col-md-9'><select name='".$am_nm[$y]."[]' class='form-control1' ><option value='0'>Select</option><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option></td>";
                            }	
							echo "</tr>";
						}
					}
