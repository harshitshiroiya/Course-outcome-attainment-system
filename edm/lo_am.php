...
<body id="mainclass" class="cbp-spmenu-push-toright">
	<div class="main-content">
		...
		<div id="page-wrapper">
		<div class="row">
			<h3 class="title1"><b>Contribution of each Assessment Method (AM) in every LO</b> (<?php echo $_SESSION['course_id']; ?>):</h3>
				<div class="form-three widget-shadow">
				<div class=" panel-body-inputin">
				<form id="co_am_form" onsubmit="return checkTotal()" action="lo_am_save.php" class="form-horizontal" enctype="multipart/form-data" method="POST">
				...
				$am_qry="SELECT `am1`,`am2`,`am3`,`am4`,`am5`,`am6`,`am7`,`am8`,`am9` FROM `lab_am` WHERE `course_id`='$course_id'";
				$lo_am_qry="SELECT * FROM `lo_am` WHERE `course_id`='$course_id'";
				//echo $co_am_qry;

				$lo_am_result = mysqli_query($conn,$lo_am_qry);
				...
				<?php
				$no=1;
				if (mysqli_num_rows($lo_am_result) > 0) 
				{
					while($lo_am_row = mysqli_fetch_assoc($lo_am_result))
					{
						$sum=0;
						echo "<tr>";
						echo "<td>LO".$no."</td>";
						...
						{
							echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$am[$y]."[]' value='".$lo_am_row[$am[$y]]."' class='form-control1' value=0 max='100' min='0' onchange='calculateSum(this)' oninput='validity.valid || (value=0);' readonly style='background-color:#dfdfdf;'></div></div></div></td>";
							$sum+=$lo_am_row[$am[$y]];
						}
	...
	<!-- Classie --><!-- for toggle left push menu script -->
		...
		<script>
			...
			function editRow(w)
			{
				...
				document.getElementById("co_am_form").action = "lo_am_update.php";
