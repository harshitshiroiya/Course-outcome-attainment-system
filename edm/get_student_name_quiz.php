...
//echo "<script>alert('$class');</script>";
    ...
    //echo implode($intake, ',');
$quiz=array( 'quiz1','quiz2','quiz3','quiz4','quiz5');
...
$count_qry = "SELECT * FROM `marks_quiz` WHERE `batch`='$batch' AND `sem_id`='$intake' AND `course_id`='$courseid'";
...
echo "<form action='marks_quiz_save.php' class='form-horizontal' enctype='multipart/form-data' method='POST'>";
...
//    echo $intake_query;
    ...
    for($y=0;$y<sizeof($quiz);$y++)
              {
                echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$quiz[$y]."[]' class='form-control1' value=0 max='20' min='0' readonly oninput='validity.valid || (value=0);'></div></div></div></td>";
              }
...
else
{
  ...
  if (mysqli_num_rows($student_result) > 0) 
    ...
    for($y=0;$y<sizeof($quiz);$y++)
              {
                echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$quiz[$y]."[]' class='form-control1' value=0 max='20' min='0'  oninput='validity.valid || (value=0);'></div></div></div></td>";
              }
         // echo "<thead><th>".$student_row['std_roll_no']."</th></thead>";
         // echo "<thead><th>".$student_row['student_name']."</th></thead>";
       }
