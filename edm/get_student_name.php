...
//echo "<script>alert('$class');</script>";
    ...
    //echo implode($intake, ',');
$experiment=array( 'exp1','exp2','exp3','exp4','exp5','exp6','exp7','exp8','exp9','exp10','exp11','exp12','exp13','exp14','exp15','exp16','exp17','exp18','exp19','exp20');
...
$count_qry = "SELECT * FROM `marks_experiment` WHERE `batch`='$batch' AND `sem_id`='$intake' AND `course_id`='$courseid'";
...
echo "<form action='marks_exp_save.php' class='form-horizontal' enctype='multipart/form-data' method='POST'>";
...
//    echo $intake_query;
    ...
    for($y=0;$y<sizeof($experiment);$y++)
              {
                echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$experiment[$y]."[]' class='form-control1' value=0 max='15' min='0' readonly oninput='validity.valid || (value=0);'></div></div></div></td>";
              }
...
else
{
  ...
  if (mysqli_num_rows($student_result) > 0) 
    ...
    for($y=0;$y<sizeof($experiment);$y++)
              {
                echo "<td><div class='form-group'><div class='col-md-10'><div class='input-group'><input type='number' name='".$experiment[$y]."[]' class='form-control1' value=0 max='15' min='0'  oninput='validity.valid || (value=0);'></div></div></div></td>";
              }
         // echo "<thead><th>".$student_row['std_roll_no']."</th></thead>";
         // echo "<thead><th>".$student_row['student_name']."</th></thead>";
       }
