...
<body id="mainclass" class="cbp-spmenu-push-toright">
	<div class="main-content">
		...
		<div id="page-wrapper">
		<div class="row">
			<h3 class="title1"><b>INFORMATION & COMMUNICATION TECHNOLOGY LO </b> (<?php echo $_SESSION['course_id']; ?>):</h3>
				<div class="form-three widget-shadow">
				<div class=" panel-body-inputin">
				<form id="lo_am_form" onsubmit="return checkTotal()" action="information_communication_lo_save.php" class="form-horizontal" enctype="multipart/form-data" method="POST">
				...
				$gap_qry="SELECT `sr_no`,`category`,`details`,`module_no`,`no_hours`,`lo_no`,`po` FROM `information_commtechnology_lo` WHERE `course_id`='$course_id'";
				$lo_gap_qry="SELECT * FROM `information_commtechnology_lo` WHERE `course_id`='$course_id'";
				//echo $co_am_qry;

				$lo_gap_result = mysqli_query($conn,$lo_gap_qry);
				...
				<?php
				$no=1;
				if (mysqli_num_rows($lo_gap_result) > 0) 
				{
					while($lo_gap_row = mysqli_fetch_assoc($lo_gap_result))
					{
						// echo implode(',',$exit);
						// echo "<br>";
						for($y=0;$y<sizeof($gap);$y++)
						{
							...
							echo "<td><input type='text' name='".$gap[$y]."[]' value='".$lo_gap_row[$gap[$y]]."' class='form-control1' value=0 max='100' min='0' onchange='calculateSum(this)' oninput='validity.valid || (value=0);' readonly style='background-color:#dfdfdf;'></td>";
