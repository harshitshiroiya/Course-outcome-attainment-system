	...
	<div class="main-content">
		...
		<div class="row">
			<h3 class="title1">List of Lab Outcome (LO) (<?php echo $_SESSION['course_id']; ?>):</h3>
				...
				<thead>
				<tr>
				<th>LO No.</th><th>Unique LO No.</th><th>LO Statement</th><th>Blooms Taxonomy Level</th><th>% weightage</th><th>PSOs</th><th>Performance Indicator (PI) of PO</th>
				</tr>
				</thead>
				<?php
				...
				<tbody>
				<tr>
				<td><div class='form-group'><div class="col-md-12"><div class="input-group"><input type="text" name="lo_no" class="form-control1" value="<?php echo 'LO'.$lo_no;?>" readonly></div></div></div></td>
				...
				<td><div class='form-group'><div class="col-md-12"><div class="input-group"><textarea name="lo_stmt" id="txtarea1" cols="50" rows="4" class="form-control1" required='required'></textarea></div></div></div></td>
				...
				<?php
				$course_id=$_SESSION['course_id'];
				$lab_qry="SELECT `course_tchr`.`trans_id`, `course_tchr`.`tchr_id`,`users`.`e_name`,`course_tchr`.`class_code`,`course_tchr`.`emailid`,`course_tchr`.`mobile`,`course_tchr`.`ext` FROM `course_tchr` INNER JOIN `users` ON `users`.`e_no`=`course_tchr`.`tchr_id` where `course_id`='$course_id' AND `term`='$term'";
				$lab_result = mysqli_query($conn,$lab_qry);
				if (mysqli_num_rows($lab_result) > 0) 
				{
					while($course_row = mysqli_fetch_assoc($lab_result))
					{
						echo "<tr>";
